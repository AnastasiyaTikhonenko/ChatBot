...
class VKBot:
    ...
    def name(self, user_id):
        ...
        try:
            ...
            self.write_msg(user_id, 'Ошибка. Пожалуйста, введите токен в переменную user_token')
    ...
    def get_age_min(self, user_id):
        ...
        try:
            ...
            if len(date_list) == 3:
                ...
                self.write_msg(user_id, 'Введите минимальный возраст для поиска кандидатов(min - 18): ')
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                        age = event.text
                        return age
        except KeyError:
            self.write_msg(user_id, 'Ошибка, пожалуйста, введите токен')

    def get_age_max(self, user_id):
        url = url = f'https://api.vk.com/method/users.get'
        params = {'access_token': user_token,
                  'user_ids': user_id,
                  'fields': 'bdate',
        ...
        try:
            ...
            if len(date_list) == 3:
                ...
                self.write_msg(user_id, 'Введите минимальный возраст для поиска кандидатов(max - 70): ')
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                        age = event.text
                        return age
    ...
    def find_city(self, user_id):
        ...
        try:
            ...
            self.write_msg(user_id, 'Ошибка, введите токен')
    ...
    def find_persons(self, user_id, offset):
        ...
        insert_data_users_seen(self.person_id(offset), offset)
        ...
        if self.get_photo_2(self.person_id(offset)) != None:
            ...
            self.write_msg(user_id, f'Больше нет фотографий')
