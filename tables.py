...
def create_table_users_seen():
    ...
    print("[INFO] Table USERS_SEEN was created.")
...
def insert_data_users_seen(vk_id, offset):
    with connection.cursor() as cursor:
        cursor.execute(
            f"""INSERT INTO users_seen (vk_id) 
            VALUES ('{vk_id}')
            OFFSET '{offset}';"""
        )
...
def drop_users_seen():
    with connection.cursor() as cursor:
        cursor.execute(
            """DROP TABLE IF EXISTS users_seen CASCADE;"""
        )
        print('[INFO] Table USERS_SEEN was deleted.')

def create_tables():
    drop_users()
    drop_seen_users()
    create_table_users()
    create_table_users_seen()
